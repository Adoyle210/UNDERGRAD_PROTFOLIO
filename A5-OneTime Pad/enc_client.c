...
int main(int argc, char *argv[]) {
  if (argc != 4){ //check args 
		printf("usage: enc_client plaintext key port\n");
		exit(1);
	}
  ...
  // set up the struct:
  memset((char*)&addr_remote, '\0', sizeof(addr_remote)); // Clear out the address struct
  ...
  if (HostInfo == NULL) { 
    fprintf(stderr, "enc_client: error, no such host error code: %d\n", errno);
    exit(1); 
  }
  ...
  if(plaintextFD != 0 && keyFD != 0) {
    ...
    char params[256];
    memset(params, '\0', 256);
    sprintf(params, "%d e ", totalSize); //e is for enc
    sendMSG(params, socketFD); //send param
    sendMSG(plaintext, socketFD); //send plaintext
    sendMSG(key, socketFD); //send key
    if(plaintextFD != NULL){ fclose(plaintextFD);}
    if(keyFD != NULL) {fclose(keyFD);}
    ...
    do{
      ...
      //printf("in enc_client: from server: \"%s\"\n", buffer);
      printf("%s", buffer);
    }while(charsRead > 0);
    printf("\n");
  ...
  if(fgets(buffer, (BUFFER_SIZE - 1), fp) != NULL) { //loop if buff filled 
    for(int i = 0; i < BUFFER_SIZE; ++i) {
      if(buffer[i] == '\n' || buffer[i] == '\0') { //break at end 
	      ...
	      fprintf(stderr, "enc_client: error plaintext contained invalid character\n");
	      exit(1);
      }
    }  
  }
