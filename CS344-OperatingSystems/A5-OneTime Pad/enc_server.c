...
int main(int argc, char *argv[]){
    ...
    while(1) {
        ...
        if(NumChildren < LISTEN_QUEUE) {
            ...
            else { //deals with encryption:
	            pid_t pid;
	            if((pid = fork()) == 0) { //create new child
                    encrypt(buffer, newsockfd); //handles encryption
...
/**************************************************************************
 * def: This function handles the encrypting portion and sends it back
****************************************************************************/
void encrypt(char* buffer, int newsockfd){
	...
	if(type == 'd') {
	    fprintf(stderr, "error: dec_client cannot use enc_server error code: %d\n", errno);
	    exit(2);
	}  
    ...
    //declare a word, encrypt the message with the key, then store in word
    char* encWord = malloc(sizeof(char) * strlen(messages[0]) + 1);
    assert(encWord != 0);
    memset(encWord, '\0', sizeof(encWord));                  
    for(int i = 0; i < strlen(messages[0]); ++i){ //encrypt the message
        if(messages[0][i] != '\n') {
            int code = getCharIndex(messages[0][i]) + getCharIndex(messages[1][i]);
            code = code % 27;
            encWord[i] = validChars[code];
        }
    }                
    //send the encrypted word back to client
    int totalCharsWritten = 0;
    int encWordLen = strlen(encWord);
    while(totalCharsWritten < encWordLen) {
        charsRead = send(newsockfd, encWord, encWordLen, 0);
        if (charsRead < 0) {
