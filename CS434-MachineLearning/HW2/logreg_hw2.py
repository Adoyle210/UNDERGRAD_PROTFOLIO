...
def main():
  ...
  test = dummyAugment(X_test)
  w = [-3.4144, 0.08, 0.4192, 0.2177, 0.2745, -0.2522, 0.0561, 0.2724, -0.0528]
  predictedY = test @ w >= 0
  test_out = np.column_stack((np.expand_dims(np.array(range(233),dtype=np.int64), axis = 1), predictedY.T))
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  llh = 0 #log of likely hood 
  for i,x in enumerate(X):
    logx = logistic(w.T @ x)
    llh += y[i] * np.log(logx) + (1 - y[i]) * np.log(1 - logx)
  return -llh
