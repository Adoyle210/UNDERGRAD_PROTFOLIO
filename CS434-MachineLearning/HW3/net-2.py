...
def main():
  ...
  predictedY = np.argmax(net.forward(X_test), axis=1)[:, np.newaxis]
  test_out = np.column_stack((np.expand_dims(np.array(range(len(predictedY)),dtype=np.int64), axis = 1), predictedY))
