...
#include "glut.h"
#include "OsuSphere.cpp" //included for sphere
...
const char *WINDOWTITLE = { "Final Project by Alexis Doyle" };
...
Display( )
{
	...
	//glCallList( BoxList );
	//Added to make mini solar system: yayayay 
#define RAD 7.f
	...
	glPushMatrix( );
		...
		SetPointLight( GL_LIGHT2,  1, 0, 0,  1, 1, 1. );
		SetPointLight( GL_LIGHT2,  0, 1, 0,  1, 1, 1. );
	...
	//glEnable(GL_TEXTURE_2D);
		...
		glRotatef((Time * 360) * 15, 1, 0., 1);
		glTranslatef(0, 60., 60);
		glRotatef(Time * 360 * 80, 0, 0, 1); //should be 180
   		glTranslatef(15, 0, 0);
		glCallList( sphereList );
	...
	//glEnable(GL_TEXTURE_2D);
		...
		glRotatef((Time * 360) * 13, 1, 0., 1);
		glTranslatef(0, 80., 80);
		glRotatef(Time * 360 * 50, 0, 0, 1);
   		glTranslatef(15, 0, 0);
		glScalef(.03,.03,.03);
		glCallList( deimosList );
	...
	//glEnable(GL_TEXTURE_2D);
		...
		glRotatef((Time * 360) * 13, 1, 0., 1);
		glTranslatef(0, 80., 80);
		glRotatef(Time * 360 * 100, 0, 0, 1);
   		glTranslatef(10, 0, 0);
		glScalef(.02,.02,.02);
		glCallList(phobosList);
...
//added to make the textures 
	width = 200;
	height = 100;
	textureMap = BmpToTexture("sunmapthumb.bmp", &width, &height);
	int level = 0;
	int ncomps = 3;
	int border = 0;
	...
	glTexImage2D(GL_TEXTURE_2D, level, ncomps, width, height, border, GL_RGB, GL_UNSIGNED_BYTE, textureMap);

	//mercury
	textureMap = BmpToTexture("mercurymapthumb.bmp", &width, &height);
	...
	//venus 
	textureMap = BmpToTexture("venusmapthumb.bmp", &width, &height);
	...
	//earth 
	textureMap = BmpToTexture("earthmapthumb.bmp", &width, &height);
	...
	//moon 
	textureMap = BmpToTexture("marsbumpthumb.bmp", &width, &height);
	...
	//mars 
	textureMap = BmpToTexture("mars-thumbnail.bmp", &width, &height);
	...
	//deimos
	textureMap = BmpToTexture("deimosbump.bmp", &width, &height);

	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1,&deimosTex);
	glBindTexture(GL_TEXTURE_2D, deimosTex);
	...
	//phobos 
	textureMap = BmpToTexture("phobosbump.bmp", &width, &height);

	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1,&phobosTex);
	glBindTexture(GL_TEXTURE_2D, phobosTex);
	...
	//jupiter 
	textureMap = BmpToTexture("jupitermapthumb.bmp", &width, &height);
...
InitLists( )
{
	...
	deimosList = glGenLists(1);
	glNewList(deimosList, GL_COMPILE);
		LoadObjFile("deimos.obj");
	glEndList();

	phobosList = glGenLists(1);
	glNewList(phobosList, GL_COMPILE);
		LoadObjFile("phobos.obj");
	glEndList();
