...
const char *WINDOWTITLE = { "Geometric Modeling By Alexis Doyle" };
...
#define NUMCURVES 6
Curve Curves[NUMCURVES];	// for pattern of curves
Curve Stem;					// for patterns that are not curves
bool Freeze; 				//freezes the animaton 
bool DotsOn;				//dots for the points
bool LinesOn;				//lines for the curves 
float Time;					//for animating 
...
void	Visibility( int );
//Added functions: 
void RotateX( Point *p, float deg, float xc, float yc, float zc ); //for rotating around the x axis
void RotateY( Point *p, float deg, float xc, float yc, float zc ); //for rotating around the y axis
void RotateZ( Point *p, float deg, float xc, float yc, float zc ); //for rotating arounf the z axis 
...
Display( )
{
	...
	//Added My attempt to make things simipler 
	int a = 0, b = 2;
	for(int i = 0; i < NUMCURVES; ++i)
	{
		...
		Curves[i].p1.x0 = -i * 1.5;
		Curves[i].p1.y0 = b;
		Curves[i].p1.z0 = a;

		Curves[i].p2.x0 = i * 1.5;
		Curves[i].p2.y0 = b;
		Curves[i].p2.z0 = b;
		...
		if(Freeze == true && i != 0){
			RotateX( &Curves[i].p0 , Time * 360 , 1, 1, 1 );
			RotateY( &Curves[i].p1 , Time * 360 , 1, 1, 1 );
			RotateX( &Curves[i].p2 , Time * 360 , 1, 1, 1 );
			RotateZ( &Curves[i].p3 , Time * 360 , 1, 1, 1 );
		}	
		if(DotsOn == true){
			glPushMatrix();
			glColor3f(1,0,1);
	...
	//added for the colors:
	float rgb[3], hsv[3];
	hsv[0] = 255;
	hsv[1] = 1;
	hsv[2] = 1;
	
	//Added code here for the curvy design stuff ayayya 
	int NUMPOINTS = 20;
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'd':
		case 'D':
			if(DotsOn == 0){DotsOn = 1;}
			else{DotsOn= 0;}
			break;

		case 'l':
		case 'L':
			if(LinesOn == 0){LinesOn = 1;}
			else{LinesOn = 0;}
			break;
...
//added functions for rotating around the x,y ans z axis
