...
const char *WINDOWTITLE = { "Project TWO -- Alexis Doyle" };
...
GLuint	BoxList;				// objects display list:
GLuint	WireFrame;	            
GLuint	Polygon;	
GLuint	Blade;	
float   RotateBlades;
...
int     InsideOutside;          //inside or outside
...
Animate( )
{
    ...
    //this is from the assignment notes idk how to do this yo
    float Time;
    ...
    RotateBlades += Time * 360;  
    
    if(RotateBlades > 360)
        RotateBlades = 0;

    //force GLUT to do a Display() call next time it is convenient

	glutSetWindow( MainWindow );
	glutPostRedisplay( ); //forces u to display
}
...
Display( )
{
    ...
    //BOX TO SEE FROM INSIDE: lol i'm basic 
    glPushMatrix();
    ...
    glRotated(90, 1,0,0);
    glRotatef(RotateBlades * 50, 0,0,1); 
    glScalef(5,5,5);
    glCallList(Blade);
    glPopMatrix();
    ...
    //second Blade:
    glPushMatrix();
    glTranslatef(0.5,2.9,9);
    glRotated(90, 0,1,0);
    glRotatef(RotateBlades * 100, 0,0,1); //two times the speed 
    glScalef(3,3,3);
    glCallList(Blade);
    glPopMatrix();
...
InitGraphics( )
{
    ...
    glutIdleFunc( Animate ); //Added to animate
