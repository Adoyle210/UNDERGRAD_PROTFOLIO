...
#include "sphere.h" // wanted to use a sphere lol 
...
const char *WINDOWTITLE = { "Lighting -- Alexis Doyle" };
...
int     width, height;
unsigned char * textureMap; 	//this is the texture of the map
GLuint	Tex;					//for shits and gigs 
...
GLuint  SphereList;				// added sphere obj
GLuint  DonutList; 				// added 3rd obj to fit project reqirements 
...
float	Time;					//time for animate 
bool    Frozen;                 // sets whether the scene is frozen
bool    Light0On;				//lights stuff
bool    Light1On;
bool    Light2On;
bool	greenOn; 
float White[3] = { 1., 1., 1. };  //added bc white is not working
...
#define RAD 7.f
	...
	glPushMatrix( );
		SetSpotLight(GL_LIGHT1, 10,5,10, 0,-1,0, 1,0,0); //cords , direct light , color
		glEnable( GL_LIGHT1 );
		glDisable( GL_LIGHTING );
		glColor3f( 1., 0., 0. );
		glPushMatrix( );
	...
	glPushMatrix( );
		SetPointLight( GL_LIGHT2,  5, 7, 0,  0, 0, 1. );
		glEnable( GL_LIGHT2 );
		glDisable( GL_LIGHTING );
		glColor3f( 0, 0, 1. );
		glPushMatrix( );
	...
	glPushMatrix();
		glEnable(GL_TEXTURE_2D);
			...
			glTexEnvf(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE, GL_MODULATE); //changed gl_replace to gl_modulate so text changes
	...
	glPushMatrix();
	//glTranslatef(0, 7, 0);
	glTranslatef((float)(RAD * cos(theta)), 7, (float)(RAD * sin(theta))); //makes the cube move with the light
	
	glScalef(0.5,0.5,0.5);	
	if(greenOn){
		glColor3f(0, 1, 0); //green
		SetMaterial( 0.f, 1.f, 0.f,  16.f );
	}
	else{
...
#endif
//adding the image on the the sphere like slide show stated 
	width = 1024;
	height = 512;
	textureMap = BmpToTexture("worldtex.bmp", &width, &height);
	int level = 0;
	int ncomps = 3;
	int border = 0;
...
InitLists( )
{
	...
	DonutList = glGenLists(1);
	glNewList(DonutList, GL_COMPILE);
		glBegin(GL_QUADS);
