...
const char *WINDOWTITLE = { "Rubix cube -- Alexis Doyle" };
...
Display( )
{
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Rainbow stairs... inspired by rainbow roads in mario kart" );
...
InitLists( )
{
	...
	glNewList( BoxList, GL_COMPILE );
		...
		glBegin( GL_QUADS );
			...
			//RUBIK CUBE TESTING FOR FUN LOL :

				/*glPushMatrix();
				int a, b, c = 0;
				int section = 1; // this for the sections of the cube (3 total)

				for(int i = 1; i < 10; i++){ 
					if(i == 9){section ++;} //goes to the next sectin after 3 * 3 face is done 
					else if(i == 18){section ++;}; // goes to next section after 3 * 3 * 2 is done 
					...
					if(section == 1){ //moving tanslation for next cube
						if(i%3 != 0){
							b ++;
						}
						else{ b = 0;}
