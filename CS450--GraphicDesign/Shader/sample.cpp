...
//
//	Author:			Joe Graphics & me (Alexis Doyle)
...
const char* WINDOWTITLE = { "Shaders Project -- Alexis Doyle" };
...
GLuint  SphereList; 			// added sphere obj for pattern	
...
float	Time; 					//Added this for animate 
bool	FragmentOn; 				//Added tp animate and freeze just the pattern
bool	VertexOn;				//Added to animate and freeze just the vertex
GLSLProgram* Pattern;          	//Added for pattern 
unsigned char* Texture;		//Added for texture from slides 
GLuint TexName;					//added for texture from slides 
...
Display()
{
	...
	//glCallList( BoxList );
	//Note/skeleton from project description:
	/*Pattern->Use( );
	A = << some constant >>
	B = << some constant >>
	...
	//Added similar to slide show display portion 
	float S0 = .5f, T0 = .5f;
	float Ds = .3f, Dt = .2f;
	float ColorR = 1.f, ColorG = 0.f, ColorB = 0.f;
	...
	Pattern->SetUniformVariable("uVertex", VertexOn);
	Pattern->SetUniformVariable("uFragment", FragmentOn);
	float A = .3, B = .3;
	if (VertexOn == true) {		//when vertex is on
		A = 360. * Time;		//time is not working lol 
		B = 360. * Time;
	}
	Pattern->SetUniformVariable("uA", A);
	Pattern->SetUniformVariable("uB", B);
	float C = .4, D = .4;
	if (FragmentOn == true) {	//when fragment is on 
		C =  C * Time;
		D =  D * Time;	
	}
	Pattern->SetUniformVariable("uC", C);
	...
	//draw something here lol  
	glCallList(SphereList);

	//added for texture 
	/*glActiveTexture( GL_TEXTURE5 ); // use texture unit 5
	glBindTexture( GL_TEXTURE_2D, TexName );
	Pattern->SetUniformVariable( "uTexUnit", 5 ); */// tell your shader program you are using texture unit 5
...
InitGraphics()
{
	...
	// added this section is from the texture slide: 
	// init glew (a window must be open to do this):
	int width, height;
	...
	glGenTextures(1, &TexName); // assign binding “handles”
	glBindTexture(GL_TEXTURE_2D, TexName); // make the Tex0 texture current and set its parameters
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, Texture);
	//end of added for texture 
...
#endif
	//added code here from slide show:
	Pattern = new GLSLProgram();
	bool vaild = Pattern->Create("pattern.vert", "pattern.frag");
	if (!vaild) {
...
InitLists()
{
	...
	//added sphere to use for shaders lol heheh lol
...
Keyboard(unsigned char c, int x, int y)
{
	...
	case 'P':
		...
		//Start of Added keys:
	case 'b':	//animate both the vertex and fragment shader patterns together 
		VertexOn = true;
		FragmentOn = true;
		break;
	case 'f':	//freeze both animations 
		...
		FragmentOn = false;
		break;
	case '1':	//animate just the fragment shader pattern and freeze the vertex
		VertexOn = false;
		FragmentOn = true;
		break;
	case '2':	//animate just the vertex and freeze the fragment 
		VertexOn = true;
		FragmentOn = false;
		break;
		// End of Added Keys!
