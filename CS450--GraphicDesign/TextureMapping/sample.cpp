...
//#include "sphere.h" // this is to draw the sphere for the planet 
...
//
//	Author:			Joe Graphics  and me kinda (I am alexis lol)
...
const char *WINDOWTITLE = { "Texture Mapping -- Alexis Doyle" };
...
int 	distortShit;			//this is for distorting shit :))
int		ActiveButton;			// current button that is down
GLuint	AxesList;				// list to hold the axes
int		AxesOn;					// != 0 means to draw the axes
int		DebugOn;				// != 0 means to print debugging info
int		DepthCueOn;				// != 0 means to use intensity depth cueing
int		TextureOn;				// I added this for texture options 
...
OsuSphere( float radius, int slices, int stacks )
{
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			/*if (TextureOn == DISTORT)
			{
				p->s = ( distortShit + M_PI  ) /( 2.*M_PI );
				p->t = ( lat + M_PI/2. ) / M_PI;
			}
			else
			{
...
Display( )
{
	...
	if(TextureOn == NOTEXTURE)
	{	
		glCallList(SphereList);	
	}

	else if (TextureOn == TEXTURE)
	{
...
void 								//added to handle texture like other menu items 
...
InitMenus( )
{
	...
	glutAddMenuEntry( "Off",  NOTEXTURE );
	glutAddMenuEntry( "On",   TEXTURE );
